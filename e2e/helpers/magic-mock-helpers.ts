  ...
  switch (mode) {
    case MagicMockMode.RECORD:
      ...
      if (!isCurrentlyRecording) {
        ...
        await expect
          .poll(async () => await page.evaluate(() => localStorage.getItem('magic-mock-recording')))
          .toBe('true')
      }
      if (isCurrentlyMocking) {
        await mockButton.click()
        await expect
          .poll(async () => await page.evaluate(() => localStorage.getItem('magic-mock-mocking')))
          .toBe('false')
      }
      break
 ...
 */
export async function verifyRequestsAreReal(
