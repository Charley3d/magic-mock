...
/**
 * E2E tests for Magic Mock functionality
 * Tests recording, mocking, and off modes with performance comparison
 *
 * Note: This test suite is designed for the vite-vue example which has
 * the Magic Mock plugin properly configured with filesystem caching.
 *
 * Performance expectations with sequential fetching (for...of loop):
 ...
 * - Expected speedup: 3-10x faster with caching (not 100x)
 */

test.describe('Magic Mock - Mode Testing', () => {
...
test.describe('Magic Mock - Recording Mode', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForPageReady(page)
    await clearMagicMockCache(page)
...
test.describe('Magic Mock - Mocking Mode', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForPageReady(page)
    await clearMagicMockCache(page)
  ...
  test('should serve cached responses in Mock mode after recording', async ({ page }) => {
    ...
    // In mock mode, we should not make new network requests
    // (or they should be intercepted and served from cache)
    // The exact behavior depends on implementation
  })
})

test.describe('Magic Mock - Performance Comparison', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForPageReady(page)
    await clearMagicMockCache(page)
  ...
  test('should demonstrate performance improvement from Record to Mock mode', async ({ page }) => {
    ...
    console.log(`Record mode time: ${recordTime}ms (sequential fetching: 10 Pokemon)`)
    ...
    console.log(`Mock mode time: ${mockTime}ms`)
    const speedupFactor = recordTime / mockTime
    console.log(`Speedup factor: ${speedupFactor.toFixed(1)}x faster`)
  ...
  test('should show consistent times in Mock mode across multiple fetches', async ({ page }) => {
    ...
    console.log(`Mock mode times: ${times.join('ms, ')}ms`)
    ...
    console.log(`Average: ${avg.toFixed(1)}ms, Variance: ${variance.toFixed(1)}`)
...
test.describe('Magic Mock - Off Mode', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForPageReady(page)
    await clearMagicMockCache(page)
